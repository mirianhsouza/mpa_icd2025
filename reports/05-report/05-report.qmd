...
subtitle: "Aula 5"
lang: pt-BR
author: "Mírian Helena de Souza Ribeiro"
date: 2025-05-08
format: 
  html:
    theme: cosmo
...
```{r}
# Transforman para o formato longo/organizado
venda_organizada <- vendas_lojas %>%
  pivot_longer(
    cols = -loja, # todas as colunas exceto "empresa"
    names_to = c("trimestre"), # nomes de duas novas colunas
    values_to = "vendas" # nome de nova coluna para receber os valores da receita
  )
...
```{r}
# Transforman para o formato longo/organizado
marketing_organizado <- investimentos_marketing %>%
  pivot_longer(
    cols = -plataforma, # todas as colunas exceto "plataforma"
    names_to = c("mês"), # nomes de duas novas colunas
    values_to = "investimentos" # nome de nova coluna para receber os valores da receita
  )

# Visualiza o resultado
marketing_organizado
```
...
incidadores_dptos_organizada <- incidadores_dptos %>%
  pivot_longer(
    ...
    names_to = c("mês", "indicador"), # nomes de duas novas colunas
    names_sep = "_", # nomes de duas novas colunas
...
```{r}
gapminder %>% # Pegue os dados gapminder, 
  filter(year == 2007) %>% # Filtre para apenas o ano 2007,
  group_by(continent) %>% # Agrupe por continente,
  summarize( # Calcule resumos:
...
```{r}
paises_asia_2007_pop50M <- gapminder %>%
  filter(
    continent == "Asia", 
    pop > 50000000, 
    year == 2007
...
```{r}
  ...
  select(country, year, pop, pop_em_milhoes) %>%
  head (5)
```
...
gapminder %>%
  ...
  select(country, year, pop, economia_grande) %>%
  head(5)
```
...
gapminder %>%
  ...
  mutate(expectativa_vida = case_when(
      ...
      lifeExp > 75 ~ "Alta" )) %>%
  print(multiplas_categorias, n= inf) %>%
  head(6)
```
...
gapminder %>%
  group_by(continent) %>%
  summarize(expectativa_vida = mean(lifeExp))
```
...
gapminder %>%
  group_by(continent) %>%
  summarize(
    numero_de_filiais = n_distinct(country),            
    pib_per_capita_medio = mean(gdpPercap),
    pib_per_capita_maximo = max(gdpPercap) 
  )
```
...
gapminder %>%
  filter(continent == "Americas") %>%
  group_by(year) %>%
  summarize(expectativa_vida_media = mean(lifeExp)) %>%
  arrange(year)
```
...
```{r}
expectativa_vida_media_continentes <- gapminder %>%
  filter(year == 2007) %>%                 
  group_by(continent) %>%                   
  summarize(expectativa_vida_media = mean(lifeExp)) %>%  
  arrange(desc(expectativa_vida_media))

# Visualiza o resultado
expectativa_vida_media_continentes
```
